...
public class FileController {
	...
	public Map<String,Object> saveFile(MultipartHttpServletRequest multiRequest) throws Throwable {
		...
		String UPLOAD_DIR="file_dbfile";//파일저장 폴더                                  
		String uploadPath="C:\\practice"+File.separator+UPLOAD_DIR;//파일저장 경로
		String saveFilePath = "";
		...
		while(iter.hasNext()) {
			...
			String originName = mFile.getOriginalFilename(); //파일 이름
			...
			if(mFile.getSize()!=0) { //file이 업로드 되면
				...
				uploadVO.setSaveFilePath(saveFilePath);
				readDbFile(uploadVO);//파일 읽어서 success, fail 분기 / 특수문자, 개수 에러처리
			}
		}//end while

		map.put("successCnt", uploadVO.getSuccessCnt()); //int
		map.put("errorCnt", uploadVO.getErrorCnt()); //int
		map.put("errorList", uploadVO.getErrorList()); //List<FileVO>
		map.put("successList", uploadVO.getSuccessList()); //List<Integer>
		
		return map;
	}
	
	
	//파일 읽어서 success, fail 분기 / 특수문자, 개수 에러처리
	private void readDbFile(UploadVO uploadVO) throws Throwable {
		
		int cnt = 0, successCnt = 0, errorCnt = 0;
		List<FileVO> successList = new ArrayList<>();
		List<FileVO> errorList = new ArrayList<>();
		
		Scanner scanner = new Scanner(new File(uploadVO.getSaveFilePath()));//들어온 dbfile 읽기         
		...
		try {
			...
			uploadVO.setCnt(errorCnt); 
			uploadVO.setSuccessCnt(successCnt); 
			uploadVO.setErrorCnt(errorCnt); 
			uploadVO.setSuccessList(successList); 
			uploadVO.setErrorList(errorList); 
